 ...
 */
export function computeRealizedVariance(prices, interval = 1) {
 ...
 */
export function computeRealizedVarianceParkinson(bars) {
 ...
 */
export function aggregateDailyRealizedVariance(intradayRVs) {
 ...
 *   / non-finite values.
 */
export function applyLogTransform(rv) {
 ...
 */
export function applyPreprocessingPipeline(prices, opts = {}) {
  const rv = computeRealizedVariance(prices, opts.interval || 1);
 ...
 *   of range.
 */
export function splitTrainTest(series, trainRatio = 0.8) {
 ...
 *   are non-positive.
 */
export function createSlidingWindows(series, windowSize, step = 1) {
  if (!Array.isArray(series)) throw new Error('series must be an array');
