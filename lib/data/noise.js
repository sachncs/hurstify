/**
 * @fileoverview Microstructure-noise correction utilities for hurstify.
 *
 * Implements three practical denoisers commonly applied to
 * high-frequency financial data:
 *
 * - {@link preaverageReturns}: Jacod et al. (2009) pre-averaging returns.
 * - {@link computeRealizedKernel}: Barndorff-Nielsen et al. (2008)
 *   realized kernel with optional kernel choice (Bartlett, Parzen,
 *   Tukey-Hanning).
 * - {@link debiasLogVolatility}: a small heuristic that nudges raw H
 *   estimates upwards to compensate for noise attenuation of the
 *   roughness measure.
 *
 ...
 *   elements.
 */
export function preaverageReturns(prices, windowSize = 2) {
 ...
 */
export function computeRealizedKernel(
  returns,
  kernelType = 'bartlett',
  bandwidth = null,
) {
  ...
  let rv = gamma[0];
  for (let k = 1; k <= h; k++) {
    const w = kernelWeight(kernelType, k, h);
    rv += 2 * w * gamma[k];
  }
 ...
 */
function kernelWeight(type, k, h) {
  const x = k / h;
  switch (type) {
    case 'bartlett':
      return 1 - x;
 ...
 */
export function debiasLogVolatility(rawHEstimates, sigmaObs, sigmaLatent) {
