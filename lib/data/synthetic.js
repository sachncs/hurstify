...
import {generateFractionalBrownianMotion} from '../stochastic-generators.js';
import {setRandomSeed, resetRandomSeed, nextRandom} from '../prng.js';
 ...
 */
export function generateVixLogVolatility(nDays, h = 0.1, opts = {}) {
  if (nDays <= 0) return [];
  if (h <= 0 || h >= 1) throw new Error('h must be in (0, 1)');

  if (opts.seed !== undefined) setRandomSeed(opts.seed);

  const fbm = generateFractionalBrownianMotion(nDays, h);
  const noiseStd = opts.noiseStd || 0.05;
  const drift = opts.drift || 0.02;

  // VIX log-vol: mean ~ 2.0 (vol ~ 20%), with rough fBM component.
  // The 0.5 factor maps a standard fBM (unit variance) into the log-vol
  // scale that matches the empirical VIX roughness regime (h ~ 0.1).
  const logVol = fbm.map((v) => {
    const noise = (nextRandom() * 2 - 1) * noiseStd;
    return 2.0 + drift * (v / Math.sqrt(nDays)) + v * 0.5 + noise;
  });

  if (opts.seed !== undefined) resetRandomSeed();
  return logVol;
}
 ...
 * @throws {Error} When `h` is out of `(0, 1)`.
 */
export function generateSpxLogVolatility(nDays, h = 0.14, opts = {}) {
  if (nDays <= 0) return [];
  if (h <= 0 || h >= 1) throw new Error('h must be in (0, 1)');
  ...
  // The 0.4 factor maps a standard fBM into the SPX-RV log-vol scale;
  // smaller than the VIX factor because SPX-RV volatility is empirically
  // smoother (h ~ 0.14) and a smaller amplitude keeps the simulated path
  // in the same regime as the empirical series.
  const logVol = fbm.map((v) => {
    const noise = (nextRandom() * 2 - 1) * noiseStd;
...
export function generateIntradayPrices(
  ...
  for (let d = 0; d < nDays; d++) {
    const fbm = generateFractionalBrownianMotion(nIntraday, h);
    const prices = [100]; // Start price
    for (let i = 1; i < nIntraday; i++) {
      const vol = Math.exp(fbm[i] * 0.5);
      const ret = drift + vol * (nextRandom() * 2 - 1) * Math.sqrt(dt);
