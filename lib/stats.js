...
export function computeKsDistance(sample1, sample2, isSorted = false) {
  ...
  if (sample1.length === 0 || sample2.length === 0) {
    throw new Error('computeKsDistance requires non-empty arrays');
  }
  if (
    sample1.some((v) => !Number.isFinite(v)) ||
    sample2.some((v) => !Number.isFinite(v))
  ) {
    throw new Error('computeKsDistance requires finite values');
  }
...
export function computeKsDistanceRescaled(sortedA, sortedB, factorA, factorB) {
  ...
  if (sortedA.length === 0 || sortedB.length === 0) {
    throw new Error('computeKsDistanceRescaled requires non-empty arrays');
  }
