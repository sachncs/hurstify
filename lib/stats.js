/**
 * @fileoverview Statistical primitives powering the Hurstify estimator.
 *
 ...
 * fully deterministic.
 *
 * @see ./hurstify.js for the estimator that consumes these primitives.
 * @see ./prng.js for the seeded random source.
 */

import {nextRandom} from './prng.js';
import {HurstifyError, HurstifyErrorCode} from './errors.js';
...
export function computeKsDistance(sample1, sample2, isSorted = false) {
  ...
  if (sample1.length === 0 || sample2.length === 0) {
    throw new HurstifyError(
      ...
      HurstifyErrorCode.EMPTY_WINDOW,
    );
  }
  if (
 ...
 */
export function computeKsDistanceRescaled(sortedA, sortedB, factorA, factorB) {
  if (!Array.isArray(sortedA) && !(sortedA instanceof Float64Array)) {
    throw new HurstifyError(
      'sortedA must be an array',
      HurstifyErrorCode.INVALID_DATA_SHAPE,
    );
  }
  if (!Array.isArray(sortedB) && !(sortedB instanceof Float64Array)) {
    throw new HurstifyError(
      'sortedB must be an array',
      HurstifyErrorCode.INVALID_DATA_SHAPE,
    );
  }
 ...
 */
export function permuteBlocks(data, blockSize, randomPhase = false) {
  ...
  if (blockSize <= 0 || blockSize > data.length) {
    throw new HurstifyError(
      ...
      HurstifyErrorCode.INVALID_BLOCK_SIZE,
    );
  }
  ...
  let offset = 0;
  if (randomPhase) {
    offset = Math.floor(nextRandom() * blockSize);
  }
 ...
 */
export function getRandomSample(array, n) {
  const len = array.length;
  if (n >= len) return shuffleArray(array);
  if (n <= 0) return [];
