/**
 * @fileoverview Random number generation utilities for hurstify.
 *
 * This module centralizes every stochastic primitive that other parts of
 * the library depend on:
 *
 * - `nextGaussian` / `generateGaussianBatch`: scalar or vector draws from
 *   the standard normal via the Box-Muller polar method.
 * - `generateCorrelatedGaussian`: pairs of correlated standard normals
 *   (used by the rBergomi simulator).
 * - `generateFractionalNoise`: fractional Gaussian noise via Hosking's
 *   method — exact at any `H in (0, 1)` but `O(n^2)`.
 * - `generateFractionalBrownianMotion`: fractional Brownian motion
 *   obtained by cumulative summation of fGN.
 * - `computeFractionalKernel` / `computeFractionalIntegral`: the
 *   Riemann-Liouville kernel and integral that drive the
 *   rough-volatility simulators.
 *
 * All routines route their randomness through `prng.js`, so calling
 * `setRandomSeed(seed)` before any of them makes the whole pipeline
 * reproducible.
 *
 * ## References
 *
 ...
 * @see ../strategies/kernel.js for the polymorphic kernel abstraction.
 */
 ...
 * and discards the sine. Use {@link generateCorrelatedGaussian} if you
...
export function nextGaussian() {
  let u = 0;
  let v = 0;
  // Guard against the (1 in 2^53) chance of `nextRandom()` returning
  // exactly 0, which would break the Box-Muller transform with
  // `log(0) = -Infinity`.
  while (u === 0) u = nextRandom();
  while (v === 0) v = nextRandom();
 ...
 * {@link nextGaussian} calls would each allocate internally.
 *
 * @param {number} n Number of samples (`n >= 0`).
 * @return {Float64Array} Buffer of `n` independent standard normals.
 */
export function generateGaussianBatch(n) {
  const result = new Float64Array(n);
  for (let i = 0; i < n; i++) {
    result[i] = nextGaussian();
  }
  return result;
}
 ...
 */
export function generateCorrelatedGaussian(n, rho) {
 ...
 */
export function generateFractionalNoise(n, H) {
  ...
  for (let i = 1; i < n; i++) {
    ...
    x[i] = mean + Math.sqrt(Math.max(0, v)) * nextGaussian();
  }
 ...
 *
 * The implementation delegates the heavy lifting to {@link generateFractionalNoise}
 ...
 * @throws {Error} When `H` is out of range (propagated from `generateFractionalNoise`).
 */
export function generateFractionalBrownianMotion(n, H) {
  if (n <= 0) return new Float64Array(0);

  const fgn = generateFractionalNoise(n, H);
 ...
 */
export function computeFractionalKernel(H, nSteps, dt) {
 ...
 * {@link computeFractionalKernel}, returns
 ...
 */
export function computeFractionalIntegral(dW, kernel, t) {
  let sum = 0;
  for (let j = 0; j < t; j++) {
