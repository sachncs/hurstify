...
export class HoltWintersForecaster extends Forecaster {
  ...
  predict(series) {
    if (!series || series.length === 0) {
      throw new HurstifyError(
        'HoltWintersForecaster: series must be non-empty',
        HurstifyErrorCode.INVALID_FORECASTER_INPUT,
      );
    }
    if (this.alpha < 0 || this.alpha > 1) {
      throw new HurstifyError(
        'HoltWintersForecaster: alpha must be in [0, 1]',
        HurstifyErrorCode.INVALID_FORECASTER_INPUT,
      );
    }
    if (this.beta < 0 || this.beta > 1) {
      throw new HurstifyError(
        'HoltWintersForecaster: beta must be in [0, 1]',
        HurstifyErrorCode.INVALID_FORECASTER_INPUT,
      );
    }
    let level = series[0];
...
export class LstmForecaster extends Forecaster {
  ...
  predict(series) {
    if (!series || series.length === 0) {
      throw new HurstifyError(
        'LstmForecaster: series must be non-empty',
        HurstifyErrorCode.INVALID_FORECASTER_INPUT,
      );
    }
    ...
    return Math.max(0.01, Math.min(0.99, h[0]));
  }
}
...
export class AttentionForecaster extends Forecaster {
  ...
  predict(series) {
    if (!series || series.length === 0) {
      throw new HurstifyError(
        'AttentionForecaster: series must be non-empty',
        HurstifyErrorCode.INVALID_FORECASTER_INPUT,
      );
    }
    ...
    return Math.max(0.01, Math.min(0.99, result[0]));
  }
}
