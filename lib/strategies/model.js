...
export class RoughBergomiModel extends StochasticModel {
  ...
  price(sim, opts = {}) {
    ...
    const T = sim.times[sim.times.length - 1];
    const dt = T / nSteps;
    const prices = [];
    const volatilities = [];
...
export class RoughFsvModel extends StochasticModel {
  simulate(opts = {}) {
    ...
    for (let t = 1; t <= nSteps; t++) {
      ...
      // The 0.1 factor below is an empirical amplitude scaler that keeps
      // the rough-compensation term subdominant to the volDiff diffusion;
      // the simulator otherwise overstates the roughness contribution.
      volPath[t] = Math.max(
        1e-6,
