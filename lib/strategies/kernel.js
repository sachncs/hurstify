...
export class RiemannLiouvilleKernel extends Kernel {
  ...
  constructor(H) {
    ...
    if (!Number.isFinite(H) || H <= 0 || H >= 1) {
      throw new HurstifyError(
        'RiemannLiouvilleKernel: H must satisfy 0 < H < 1',
        HurstifyErrorCode.INVALID_KERNEL_INPUT,
      );
    }
    this.H = H;
...
export class TimeVaryingKernel extends Kernel {
  ...
  constructor(hPath, dt = 1) {
    ...
    if (!hPath || hPath.length === 0) {
      throw new HurstifyError(
        'TimeVaryingKernel: hPath must be a non-empty array',
        HurstifyErrorCode.INVALID_KERNEL_INPUT,
      );
    }
    if (!Number.isFinite(dt) || dt <= 0) {
      throw new HurstifyError(
        'TimeVaryingKernel: dt must be positive',
        HurstifyErrorCode.INVALID_KERNEL_INPUT,
      );
    }
    this.hPath = hPath;
