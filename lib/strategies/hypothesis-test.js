...
import {kalmanStep} from '../inference/filtering.js';
...
export class HypothesisTest {
  ...
  run(data, opts = {}) {
    throw new HurstifyError(
      ...
      HurstifyErrorCode.ABSTRACT_NOT_IMPLEMENTED,
    );
  }
}
  ...
  run(data, opts = {}) {
    ...
    if (!Number.isFinite(D) || D < 0) {
      throw new HurstifyError(
        ...
        HurstifyErrorCode.INVALID_HYPOTHESIS_TEST_INPUT,
      );
    }
 ...
 *
 * Delegates the predict/update math to {@link kalmanStep} so this
 * implementation stays in lockstep with {@link runKalmanFilter}.
 *
...
function kalmanLogLikelihood(observations, q, r) {
  ...
  for (let i = 0; i < n; i++) {
    if (i > 0) {
      ...
      const innovation = observations[i] - step.xPred;
      const innovationVar = step.pPred + r;
      logLikelihood +=
        -0.5 *
