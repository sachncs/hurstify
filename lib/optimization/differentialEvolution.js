...
import {shuffleArray} from '../stats.js';
 ...
 *   dim)`).
 * @param {number=} opts.cr Per-coordinate crossover probability
 *   (default `0.7`).
 ...
 *   objective value.
 */
export function runDifferentialEvolution(f, x0, opts = {}) {
  ...
  for (let i = 0; i < popSize; i++) {
    population.push(x0.map((val, j) => lb[j] + nextRandom() * (ub[j] - lb[j])));
  }
  ...
  for (let iter = 0; iter < maxIter; iter++) {
    for (let i = 0; i < popSize; i++) {
      ...
      const jRand = Math.floor(nextRandom() * dim);

      for (let j = 0; j < dim; j++) {
        if (j === jRand || nextRandom() < cr) {
...
/**
 * Optimizer strategy that wraps differential evolution for 1-D
 * objectives by lifting the scalar `objective` to a 1-D vector wrapper.
 */
import {Optimizer} from './optimizer.js';

export class DifferentialEvolutionOptimizer extends Optimizer {
  /** @override */
  minimize(objective, lower, upper, initial) {
    return runDifferentialEvolution((x) => objective(x[0]), [initial || 0.5], {
      maxIter: 300,
      popSize: 30,
    }).x[0];
  }
